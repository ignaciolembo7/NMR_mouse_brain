...
for g, roi in zip(gs, rois): 
    ...
    ax1.plot(grad, sigma, 'o-', markersize=9, linewidth=2, label=f"{roi}")
    ...
    fig1.tight_layout()
    fig1.savefig(f"{directory}/{roi}_sigma_vs_g_{g}.png", dpi=600)
    fig1.savefig(f"{directory}/{roi}_sigma_vs_g_{g}.pdf")

    #ax2.errorbar(tnogse, sigma,  fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{i}") #yerr=error_t_c,
    ax2.plot(grad, sigma, 'o-', markersize=9, linewidth=2, label=f"{roi}")
...
fig2.tight_layout()
fig2.savefig(f"{directory}/sigma_vs_g_allG.png", dpi=600)
fig2.savefig(f"{directory}/sigma_vs_g_allG.pdf")