...
for g, roi in zip(gs, rois): 
    ...
    ax1.errorbar(grad, t_c, fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{roi}") #  yerr=error_t_c,
    ...
    fig1.tight_layout()
    fig1.savefig(f"{directory}/{roi}_tauc_vs_g_{g}.png", dpi=600)
    fig1.savefig(f"{directory}/{roi}_tauc_vs_g_{g}.pdf")

    ax2.errorbar(grad, t_c,  fmt='o-', markersize=3, linewidth=2, capsize=5, label=f"{roi}") #yerr=error_t_c,
    #ax.plot(tnogse, tau_c, 'o-', markersize=3, linewidth=2)
...
fig2.tight_layout()
fig2.savefig(f"{directory}/tauc_vs_g_allG.png", dpi=600)
fig2.savefig(f"{directory}/tauc_vs_g_allG.pdf")