...
def pgse_params(method_path):
    with open(method_path) as file:
        ...
        return { "DwBvalEach": DwBvalEach[0], "DwEffBval": DwEffBval[0], "DwGradAmp": DwGradAmp[0], "DwGradRead": DwGradRead[0], "DwGradDur": DwGradDur[0], "DwGradSep": DwGradSep[0], "EchoTime": EchoTime}
...
def generate_pgse_vs_bval_roi(image_paths, method_paths, mask, slic):
    ...
    DwBvalEach, DwEffBval, DwGradAmp, DwGradRead, DwGradDur, DwGradSep, TE= np.array(params).T
    ...
    return DwBvalEach, DwEffBval, DwGradAmp[0], DwGradRead[0], DwGradDur, DwGradSep, f

def plot_nogse_vs_x_data(ax, nroi, DwEffBval, f, DwGradDur, DwGradSep, DwGradRead, slic):
    ...
    title = ax.set_title(f"G = {DwGradRead*1466.6153} mT/m  || slice = {slic} ", fontsize=18)
    #plt.tight_layout()
