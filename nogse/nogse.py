#NMRSI - Ignacio Lembo Ferrari - 5/12/2023
...
def upload_data_delta_M(file_name, roi_set):
    ...
    #ax.axis("off")
    #title = ax.set_title("$T_\mathrm{{NOGSE}}$ = {} ms  ||  $g$ = {} mT/m\n  $n$ = {}".format(T_nogse[0], g[0], n[0]), fontsize=20)
    #plt.savefig(f"../results_{file_name}/brute/contraste_NOGSE_vs_g_{modelo}/T_NOGSE={T_NOGSE}/image0_t={T_NOGSE}.png", dpi=600)
    #print("Roi set: \n", roi_set)
    ...
    f1, f2, f3, f4, f5 = contrast_matrix.T # divido en rois

    #Combinar los vectores usando zip()
    vectores_combinados = zip(g, f1, f2, f3, f4, f5)
    # Ordenar los vectores combinados basándote en vector_g
    vectores_ordenados = sorted(vectores_combinados, key=lambda x: x[0])
    # Separar los vectores nuevamente
    g_contrast, f1, f2, f3, f4, f5 = zip(*vectores_ordenados)

    return T_nogse[0], g_contrast, n[0], f1, f2, f3, f4, f5
